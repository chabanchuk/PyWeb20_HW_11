...
from sqlalchemy import or_, select, func, between
...
async def get_upcoming_birthdays(db: AsyncSession):
    ...
    contacts = await db.execute(
        select(Contact).where(
            func.date_trunc('day', Contact.birth_date).between(
                func.date_trunc('day', current_date), 
                func.date_trunc('day', end_date)
            )
        )
    )
